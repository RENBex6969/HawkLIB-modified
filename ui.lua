...
local GetService; GetService = function(ser)
  return cloneref(game:GetService(ser))
end
...
local input = GetService("UserInputService")
...
function HawkLib:Window(Win)
	if not _Hawk == "ohhahtuhthttouttpwuttuaunbotwo" then
		local ScriptName;
			...
			function ContainerItems:Section(Title)
				...
				if x ~= nil then
					...
					function SectionItems:KeyBind(keytext, first, callback)
						...
						Bind.MouseButton1Click:Connect(
							function(e)
								if not focusing then
									Bind.Text = ". . ."
									local a, b = GetService("UserInputService").InputBegan:wait()
						...
						GetService("UserInputService").InputBegan:connect(function(current, ok)
							if not ok then
								...
								if checkfile == false then
									writefile(tostring(writeinto).."\\"..tostring(filename), game:HttpGet(tostring(imagelink),true))	
										...
										if not isfile(tostring(writeinto).."\\"..tostring(filename)) then
											writefile(tostring(writeinto).."\\"..tostring(filename), HttpGet(tostring(imagelink),true))					
										end									
									end			
								end		
							end
